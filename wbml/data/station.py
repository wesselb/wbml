...
def load():
    ...
    # Extract dates.
    per_month_dates = [datetime.date(year=x['year'].iloc[0],
...
def stats_and_vis():
    ...
    out.kv('Number of dates with 20 measurements or more',
           np.sum(num_measurements >= 20))
    out.kv('Average number of measurements',
           int(np.mean(num_measurements)))
    ...
    for i, ind in enumerate(inds):
        ...
        plt.scatter(per_month[ind]['lon'], per_month[ind]['lat'],
                    s=4,

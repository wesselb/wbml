...
def load():
    ...
    for site in sites:
        for day in days:
            df = pd.read_csv(data_path('air_temp',
                                       fn.format(site=site, day=day)))
    ...
    def make_range(lower, upper):
        ...
        return date_start + (lower - 1) * day, \
               date_start + (upper - 1) * day

    # Construct data sets of various sizes.
    ds = []
    for lower, upper in [make_range(10, 20),
                         make_range(8, 23),
                         (df_all.index.min(), df_all.index.max())]:
        df = df_all[(df_all.index >= lower) & (df_all.index <= upper)]

        test1, train = split_df(df, make_range(12, 16),
                                [('temp', 'Cam')])
        test2, train = split_df(train, make_range(14, 18),
                                [('temp', 'Chi')])

        test1_ext, _ = split_df(df, make_range(11.5, 16.5),
                                [('temp', 'Cam')])
        test2_ext, _ = split_df(train, make_range(13.5, 18.5),
                                [('temp', 'Chi')])

...
def load():
    ...
    return {'hf': {
        ...
        # (RMC):
        'temperature': _load_nc(data_path('mf_temp', 'T2_monthly.nc'),
                                ('Time', 'time'),
                                ('T2', 'temperature'),
                                ('lat', 'latitude'),
                                ('lon', 'longitude'),
        ...
        'lf': {  # The below is all re-analysis data.
            ...
            'temperature': _load_nc(data_path('mf_temp',
                                              'era5_t2m_monthly.nc'),
                                    ('time', 'time'),
                                    ('t2m', 'temperature'),
                                    ('latitude', 'latitude'),
            ...
            'wind_north': _load_nc(data_path('mf_temp',
                                             'era5_v10_monthly.nc'),
                                   ('time', 'time'),
                                   ('v10', 'wind_north'),
                                   ('latitude', 'latitude'),
            ...
            'wind_east': _load_nc(data_path('mf_temp',
                                            'era5_u10_monthly.nc'),
                                  ('time', 'time'),
                                  ('u10', 'wind_east'),
                                  ('latitude', 'latitude'),
...
def _load_nc(path, x, y, coord1, coord2, expand_coords=True):
    ...
    # Load the variables.
    df = pd.DataFrame(_get_reshape(data, y[0]),
                      index=pd.Index(_get_reshape(data, x[0]), name=x[1]))
    ...
    if expand_coords:
        ...
        coord1_values, coord2_values = \
            np.broadcast_arrays(coord1_values[:, None],
                                coord2_values[None, :])
        coords = pd.DataFrame({coord1[1]: coord1_values.reshape(-1),
                               coord2[1]: coord2_values.reshape(-1)})
    else:
        # No expansion is needed.
        coords = pd.DataFrame({coord1[1]: _get_reshape(data, coord1[0]),
...
def _fetch():
    for remote, local in [('HiFid/T2_monthly.nc', 'T2_monthly.nc'),
        ...
        resource(target=data_path('mf_temp', local),
